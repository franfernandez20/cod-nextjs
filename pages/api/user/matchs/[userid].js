...
export default function handler(req, res) {
  ...
  // ultima fecha 1612045830 --  23:30:30
  // primare fecha 1612040160 --21:56
  const ini = 1612040160
  const fin = 1612045830
  ...
  fetch(
    ...
    .then((result) => {
      ...
      if (status === "success") {
        const matchesOnRange = data.matches.filter( match => match.utcStartSeconds > ini && match.utcStartSeconds < fin )
        res.end(JSON.stringify(matchesOnRange))
      } 
